...
if __name__ == "__main__":
    ...
    # Read metadata.csv and split into train, val, and test sets
    with open(input_file, 'r') as f:
        ...
        for i, row in enumerate(reader):
            if i < train_lines:
                ...
                val_data.append(','.join(row) + '\n')
            elif train_lines + val_lines <= i < train_lines + val_lines + test_lines:
                test_data.append(','.join(row) + '\n')
            else:
                break

    # Write train, val, and test sets to separate files
    write_lines(train_data, output_train_file)
    write_lines(val_data, output_val_file)
    write_lines(test_data, output_test_file)
...
## Training on CPU
    ...
    model = GradTTS(nsymbols, 1, None, n_enc_channels, filter_channels, filter_channels_dp, 
                    ...
                    n_feats, dec_dim, beta_min, beta_max, pe_scale, n_timesteps)
