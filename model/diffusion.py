...
class Diffusion(BaseModule):
    ...
    def forward_diffusion(self, x0, mask, mu, t, n_timesteps):
        time = t.unsqueeze(-1).unsqueeze(-1)
        cum_noise = get_noise(time, self.beta_min, self.beta_max, n_timesteps, cumulative=True)
    ...
    def reverse_diffusion(self, z, mask, mu, n_timesteps, stoc=False, spk=None):
        ...
        for i in range(n_timesteps):
            t = (1.0 - (i + 0.5)*h) * torch.ones(z.shape[0], dtype=z.dtype, 
                                                 device=z.device)
            time = t.unsqueeze(-1).unsqueeze(-1)
            noise_t = get_noise(time, self.beta_min, self.beta_max, n_timesteps, 
                                cumulative=False)
    ...
    def loss_t(self, x0, mask, mu, t, spk=None, n_timesteps=50):
        xt, z = self.forward_diffusion(x0, mask, mu, t, n_timesteps)
        time = t.unsqueeze(-1).unsqueeze(-1)
        cum_noise = get_noise(time, self.beta_min, self.beta_max, n_timesteps, cumulative=True)
    ...
    def compute_loss(self, x0, mask, mu, spk=None, offset=1e-5, n_timesteps=50):
        t = torch.rand(x0.shape[0], dtype=x0.dtype, device=x0.device,
                       requires_grad=False)
        t = torch.clamp(t, offset, 1.0 - offset)
        return self.loss_t(x0, mask, mu, t, spk, n_timesteps)
